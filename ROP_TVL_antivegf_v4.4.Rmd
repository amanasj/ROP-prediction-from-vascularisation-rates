...
df_orig <- suppressMessages(read_excel(("../data/data_v3.0/ROP_070224 antivegf clean.xlsx"), 
                                        sheet = 1, 
...
```{r include=FALSE}
### Some data manipulation to label the pre vs post points properly
####################################################################################################
...
As per Caroline's great suggestion here is a plot of the slope values before and after anti-vegf shown two ways. The first is a violin plot with lines connecting each patient pre- to post- treatment which helps to show each patients trajectory. The second plot is a more traditional error bar plot:
...
As we've discussed the above result of "no significant change in TVR after anti-VEGF" is a key finding of our work here. However, this could either be a null result or an insufficiently powered study. So to attempt to answer this (since a reviewer would surely question this), I am including a power calculation below.
...
update:-  Let's ignore the previous power calculation based on the slope analysis "pilot" study and reverse it by asking at 90% power, what reasonable change in slope can we hope to detect with the sample size we have and an alpha level = 0.05?
Doing it this way round means we don't use any of the "pilot" data at all and simply ask what level of slope change is detectable at 90% power compared to a null model where there is no treatment effect at all. The power calculation I am performing here runs hundreds of simulations-based power analysis with lme4 mixed modelling in order to test the null hypothesis.
