...
tvl_train_data_orig <- suppressMessages(read_excel(("XXXXXXX.xlsx"), 
                                        sheet = 1, 
                                        range = "a1:h400", col_names = T))

tvl_antivegf_data_orig <- suppressMessages(read_excel(("XXXXXXX.xlsx"),
                                        sheet = 1, 
                                        range = "a1:n257", col_names = T))

tvl_test_data_orig <- suppressMessages(read_excel(("XXXXXXX.xlsx"),
                                        sheet = 1, 
...
```{r}
# add a simple TVL ~ age slope column to dataframe. Go back to the original dataframes with the R & L on the ID labels. 
## NOTE:- our previous work here and the antivegf file looks at TVR being the coefficient of the linear mixed model whereas here TVR is manually fitted and evaluated as the coefficient of PMA (age). The two versions of the TVR calc are similar but slighty different. This one is more appropriate for predictions using gradient boosting since this TVR does not involve GA and BW at all so can be evaluated as a predictive measure independently. In all other instances TVR is calculated as the coefficient of PMA and controlling for GA and BW in the LMM. I guess since we are calculating TVR on a per patient basis here we are also controlling for GA and BW so I guess the two versions of TVR are probably the same?  
...
Random forests do not take into account the repeated measures aspect of the data (right eye & left eyes taken from each patient), whereas in the LMM I did take this into account. This limitation of random forests cannot be ignored with our data so use XGboost instead. Ttree boosting has been used to mitigate the random components of a model whilst boosting the fixed effects. 


The R package `GPboost` takes things a little further by combining tree boosting with grouped random effects (such that we can now use data from right and left eyes for each patient an take this repeated measures per patient into account).
...
fixed3 <- model.matrix(ROP ~ TVR, data = gp_data)
...
bst2 <- gpboost(data = fixed2, label = gp_data$ROP, gp_model = gp_model2, nrounds = 127, 
               learning_rate = 0.5, min_data_in_leaf = 10, max_depth=1, seed=22, lambda_l2=1, 
                verbose = 1)
bst3 <- gpboost(data = fixed4, label = gp_data$ROP, gp_model = gp_model4, nrounds = 100,
...
plot(roc_object3, print.auc=F, col="darkgreen", add=TRUE, lty=2, lwd=2)
...
text(0.3, 0.35, paste("Model 1: (TVR only)  AUC = ", round(roc_object3$auc, 2)), col="darkgreen", cex=0.7)
} 
...
So using gradient tree boosting rather than random forests takes the nested nature of the data into account in a similar way that the linear mixed modelling took the nesting into account.


This model shows that BW is overwhelmingly the strongest predictor. Again, we also have TVR adding some contribution.  
...
Let's use the withheld dataset to make predictions to test the gradient boosting model 
...
fixed3 <- model.matrix( ~ TVR, data = test_df)
...
predicted_classes3 <- ifelse(predictions3$fixed_effect < -0.8, "AB", "C")
